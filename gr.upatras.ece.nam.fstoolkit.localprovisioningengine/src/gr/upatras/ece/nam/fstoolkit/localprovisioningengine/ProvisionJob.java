...
public class ProvisionJob extends Job {
	...
	public ProvisionJob(String name, RequestedFederationScenario scenario, 
		...
		switch (provisionJobAction) {
			case PROVISION: scenario.setStatus(ScenarioStatus.SCHEDULED_PROVISIONING);break;
			case SHUT_DOWN: scenario.setStatus(ScenarioStatus.SCHEDULED_SHUT_DOWN);break;
			case SHUT_DOWN_RESOURCE: scenario.setStatus(ScenarioStatus.PROVISIONING);break;
		}
	...
	private IStatus runProvisionJob(IProgressMonitor monitor) {
		...
		this.requestedScenario.setStatus(ScenarioStatus.PROVISIONING  );
		...
		do {
			...
			if (monitor.isCanceled()){
				...
				this.requestedScenario.setStatus(ScenarioStatus.REJECTED );
		...
		monitor.done();
		this.requestedScenario.setStatus(ScenarioStatus.RUNNING );
	...
	private IStatus runShutDownJob(IProgressMonitor monitor) {
		...
		this.requestedScenario.setStatus(ScenarioStatus.SHUTTING_DOWN  );
		...
		monitor.done();
		this.requestedScenario.setStatus(ScenarioStatus.SHUT_DOWN );
		LocalRuntimeScenariosMgt.getInstance().SaveRunningScenarioModel(this.requestedScenario);//since completed 
