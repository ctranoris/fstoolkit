 ...
 *  
 */
public class PanlabGWClient {
	private static String panlabGWAddress = PanlabServices.PanlabGWCAlias;
	private static PanlabGWClient pgwInstance;
	private static String userAgent="FCI 1.0.0";
	InputStream response_stream;

	public PanlabGWClient(String tgwaddr) {
		super();
		panlabGWAddress = tgwaddr;
	}

	/**
	 * @author ctranoris
	 * @return a PanlabGWClient Singleton
	 * 
	 */
	public static PanlabGWClient getInstance() {
		if (pgwInstance == null) {
			pgwInstance = new PanlabGWClient(
					panlabGWAddress); //just hard code for now
		}
