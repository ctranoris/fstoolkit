...
import gr.upatras.ece.nam.fstoolkit.providerAdapters.RunningScenariosContentProvider;
import gr.upatras.ece.nam.fstoolkit.providerAdapters.RunningScenariosLabelProvider;
...
implements ITabbedPropertySheetPageContributor{ //we need this to display the selected item to the Tabbed property sheet editor
	 ...
	 */
	public static final String ID = "gr.upatras.ece.nam.fstoolkit.views.RunningScenariosView";
		...
		column.setLabelProvider(new ColumnLabelProvider(){
			public String getText(Object element){
				if ( (element instanceof ui_TaxonomyTreeItem) && 
						( ((ui_TaxonomyTreeItem)element).getBrokerElement() instanceof ResourceRequest ) ) {
					ResourceRequest resReq= (ResourceRequest) ((ui_TaxonomyTreeItem)element).getBrokerElement() ;
		...
		column.setLabelProvider(new ColumnLabelProvider(){
			public String getText(Object element){
				if ( (element instanceof ui_TaxonomyTreeItem) && 
					...
					ResourceRequest resReq= (ResourceRequest) ((ui_TaxonomyTreeItem)element).getBrokerElement() ;
					String officeName = 
						((Broker)resReq.getRefOfferedResource().getImplOfferedService().eContainer()).getName() ;
	...
	public void setFocusOnSelectedScenario( RequestedFederationScenario sourceFedScenario ){
		...
		for (TreeItem iterable_element : viewer.getTree().getItems() ) {
			if (iterable_element.getData() instanceof ui_TaxonomyTreeItem)
				if ( ((ui_TaxonomyTreeItem)iterable_element.getData()).getBrokerElement()!=null  )
					if ( ((ui_TaxonomyTreeItem)iterable_element.getData()).getBrokerElement() instanceof RequestedFederationScenario ){
						RequestedFederationScenario tempScen = (RequestedFederationScenario) (((ui_TaxonomyTreeItem)iterable_element.getData()).getBrokerElement());
...
//					}// try
	...
	private static final String IPROVISIONINGJOB_ID = "gr.upatras.ece.nam.fstoolkit.extensionpoint.provisioningjoblistener";
