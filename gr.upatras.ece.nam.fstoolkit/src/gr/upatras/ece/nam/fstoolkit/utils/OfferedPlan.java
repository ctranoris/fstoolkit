...
public class OfferedPlan{
	...
	public void createProposedScenario() {
		this.proposedScenario = 
			brokermodel.federationscenarios.FederationscenariosFactory.eINSTANCE.createRequestedFederationScenario();
		this.proposedScenario.setName( sourceFedScenario.getName());
		
		InfrastructureRequest infReq = brokermodel.federationscenarios.FederationscenariosFactory.eINSTANCE.createInfrastructureRequest();
		this.proposedScenario.setInfrastructureRequest(infReq );
		
		
		for (Iterator<brokermodel.federationscenarios.Import> iterator =  sourceFedScenario.getImports().iterator(); iterator.hasNext();) {
			brokermodel.federationscenarios.Import imp = (brokermodel.federationscenarios.Import) iterator.next();
			brokermodel.federationscenarios.Import importval = brokermodel.federationscenarios.FederationscenariosFactory.eINSTANCE.createImport();
		...
		if ( sourceFedScenario.getScheduledPlan()!=null ){				
			ScheduledPlan splan = brokermodel.federationscenarios.FederationscenariosFactory.eINSTANCE.createScheduledPlan() ;
			...
			if (servReq instanceof ServiceRequest) {
				...
				if (servReq.getNumOfServices()>0 ){
					...
					ResourceGroup  resGroup = 
						brokermodel.federationscenarios.FederationscenariosFactory.eINSTANCE.createResourceGroup() ;
	...
	private ResourceRequest  addResourceForService(ServiceRequest serviceReq, String proposedResourceName) {
		...
		Broker tempoffice = (brokermodel.Broker) serviceReq.getRefService().eContainer();
...
//			}
	...
	private AdvicedOffer AddOfferForContract( Broker tempoffice,
		...
		arg0.setForBroker(tempoffice.getName());
		System.out.println("Advice add=" + arg0.getId() +" for office "+tempoffice.getName() );
		//create a resourceRequest
		ResourceRequest reqResource = brokermodel.federationscenarios.FederationscenariosFactory.eINSTANCE.createResourceRequest();
...
//	}
	...
	public void addSettingsToResourceRequest(ResourceRequest resourceRequest, ServiceRequest serviceRequest){
		...
		for (int j = 0; j < offeredResource.getResourceSettings().size() ; j++) {
			...
			ResourceSettingInstance resSetInst = brokermodel.federationscenarios.FederationscenariosFactory.eINSTANCE.createResourceSettingInstance() ;
...
//									splitOldIP = igw.split( "\\." )  ;
		...
		for (AdvicedOffer offer : this.getAdvicedOffers()) {
			...
			for (ResourceSettingInstance settingToInvestigate : resourceReqInvestigated.getReqResourceSettings()) {
				...
				for (AbstractSetting s : setting.getRequiresParams()) {
					...
					//site.partner.office
					Broker tempoffice = (Broker)(resourceRequired.eContainer().eContainer().eContainer());
		...
		for (AdvicedOffer offer : this.getAdvicedOffers()) {
			for (Resource resource : offer.getOfferedResource().getRequiresResources()) {
				...
				if (CheckIfResourceRequestExistsInPlan(resource, null)==null){
					Broker tempoffice = (Broker)(resource.eContainer().eContainer().eContainer());
	...
	private ServiceRequest tryToFindExistingServiceRequest(Broker tempoffice,
	...
	public ResourceServiceContract findContractForResource(Broker tempoffice, Resource resource){
