...
//		System.out.println("scope_ResourceSettingInstance_refResourceSetting ctx.eContainer()="+ ctx.eContainer().toString());
		ResourceRequest offeredResourceInstance = (ResourceRequest) ctx
				.eContainer();
		FederationOffice.resources.OfferedResource offeredResource = offeredResourceInstance
				.getRefOfferedResource();

		// return new SimpleScope(IScope.NULLSCOPE, getOfServs(rv) );
		EList<FederationOffice.resources.ResourceSetting> features = offeredResource
				.getResourceSettings();
...
//		ctx.getRefServiceSetting()
		...
		EList<FederationOffice.services.ServiceSetting> features = offeredService.getServiceSettings() ;
		return scopeFor(Iterables.filter(features, rServiceSettings));
		
	}

	protected Predicate<FederationOffice.services.ServiceSetting> rServiceSettings = new Predicate<FederationOffice.services.ServiceSetting>() {
		public boolean apply(FederationOffice.services.ServiceSetting input) {
			return true;
			// return input.getName().length()==6;
		}
	};
