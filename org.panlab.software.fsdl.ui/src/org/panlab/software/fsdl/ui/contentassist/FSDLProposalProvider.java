...
//				);
		...
		FederationOffice.services.Service  offeredService = null;
	...
	public void completeServiceSettingInstance_StaticValue(ServiceSettingInstance model,
		...
		if ( !( model.getRefServiceSetting().getSettingType() instanceof tideTypeEnum ))
			return;
		
		tideTypeEnum enumtype = (tideTypeEnum)model.getRefServiceSetting().getSettingType();
		...
		for (tideTypeEnumItem item : enumtype.getTideEnumlist()) {
	...
	public void completeServiceRequest_RefService(EObject model, Assignment assignment, ContentAssistContext context, 
		...
		org.eclipse.xtext.scoping.IScope scope = provider.getScope(model, FederationOfficePackage.eINSTANCE.getOffice_OfferedServices() );
...
//		System.out.println(">>>>>>>>> scoping model object=" + element.toString());
//		System.out.println(">>>>>>>>> scoping model object=" + element.getEObjectOrProxy().toString());
...
//				);
		...
		FederationOffice.resources.OfferedResource  offeredResource = null;
	...
	public void completeResourceRequest_RefOfferedResource(EObject model,
		...
		org.eclipse.xtext.scoping.IScope scope = provider.getScope(model,
					FederationOfficePackage.eINSTANCE.getOffice_RegisteredUsers());

		// scope contains only customer
		for (IEObjectDescription element : scope.getAllContents()) {
			
			OfficeUser user = (OfficeUser) element.getEObjectOrProxy();
