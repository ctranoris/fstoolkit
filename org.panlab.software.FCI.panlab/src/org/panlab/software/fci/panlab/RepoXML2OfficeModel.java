 ...
 *
 */
public class RepoXML2OfficeModel {
	private boolean displayLog = true;

	private Office office;
//	private static RepoXML2OfficeModel instance;

	public  Office getOffice() {
		return office;
	}

	private static String OfficeUsername;
	private static String OfficePassword;
...
//	}
	...
	/**
	 * The constructor of the class {@link RepoXML2OfficeModel}. The repository url is hardcoded for now
	 * @param requsername
	 * @param reqpassword
	 * @param reqVCT
	 * @author ctranoris
	 */
	public RepoXML2OfficeModel(String requsername, String reqpassword) {
		super();
	...
	private void InitModel(){
		...
		office.setAPIGateway(PanlabServices.PanlabGWCAlias);
	...
	private boolean organizationExists(String organName) {
		for (OfficeUser user : office.getRegisteredUsers()) {
...
//			}
		...
		AddVCTsOfUserAndShared( OfficeUsername );
...
//		cost.setPerUnit(unit );
	...
	public Office AddVCTsOfUserAndShared(String requsername){
		...
		TestbedDesigner eUser = FederationOffice.users.UsersFactory.eINSTANCE.createTestbedDesigner()  ;
		eUser.setName(requsername);
		eUser.setId(Integer.parseInt(personID));	
		eUser.setUniqueID("user_"+Integer.parseInt(personID));
		office.getRegisteredUsers().add(eUser );
		...
		for (int i = 0; i < vctSpecNodes.getLength(); i++) {
			RequestedFederationScenario reqFS = FederationOffice.federationscenarios.FederationscenariosFactory.eINSTANCE.createRequestedFederationScenario();
...
//			log( "vctSpecNodes.item(i).toString()  =" + vctSpecNodes.item(i).toString() );
			...
			if ( (shared.equals( "false" ))  || shared.equals( "true" ) //for now we just show ALL the VCTs in repo!
				...
				office.getAvailableFederationScenarios().add(reqFS);
...
//					RunningScenario rs = FederationOfficeFactory.eINSTANCE.createRunningScenario();
...
//					office.getRunningScenarios().add(rs);
	...
	public void LoadFSbyVCT(RequestedFederationScenario fs){
		...
		InfrastructureRequest infrReq = FederationOffice.federationscenarios.FederationscenariosFactory.eINSTANCE.createInfrastructureRequest();
		...
		for (int i = 0; i < resInstancesNodesOfVCT.getLength(); i++) {
			...
			refOfferedResource = FindOfferedResourceFromID( (Office)fs.eContainer(), resSpecID, ptmQualName );
	 ...
	 * @author ctranoris
	 */
	private OfferedResource FindOfferedResourceFromID(Office o, String resSpecID, String ptmQualName) {
	...
	public Boolean checkAuthentication() {
		...
		NodeList personSpecNodes = xmlutl.getNodeListFromObject( personDoc, "//person[userName='" + OfficeUsername + "']");
		String personID = xmlutl.getNodeValueFromObject(personSpecNodes.item(0), "@id");
		String personPass = xmlutl.getNodeValueFromObject(personSpecNodes.item(0), "password/text()");
		
		OfficeUser eUser = FederationOffice.users.UsersFactory.eINSTANCE.createOfficeCustomer() ;
		eUser.setName(OfficeUsername);
		eUser.setId(Integer.parseInt(personID));
		eUser.setUniqueID("user_"+Integer.parseInt(personID));
		Account account = FederationOffice.users.UsersFactory.eINSTANCE.createAccount();
		account.setUsername(OfficeUsername);
		
		try {
			System.out.println("personPass = "+personPass);
			System.out.println("reqpasswordMD5 = "+MungPass (OfficePassword));
			if (personPass.equalsIgnoreCase( MungPass (OfficePassword)) )
				return true;
